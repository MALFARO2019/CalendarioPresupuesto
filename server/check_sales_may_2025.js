...
async function main() {
    ...
    try {
        ...
        console.log('\n=== Sales for Avenida Escazu (S22) May 2025 ===');
        const salesMay25 = await pool.request().query(`
            ...
            WHERE CODALMACEN = 'S22' AND ANO = 2025 AND MES = 5
            GROUP BY CAST(FECHA AS DATE), DATENAME(WEEKDAY, FECHA)
            ORDER BY Fecha
        `);
        console.table(salesMay25.recordset.map(r => ({
