-- Migrar tabla MODELO_PRESUPUESTO_AJUSTES
IF NOT EXISTS (
    SELECT 1 
...
BEGIN
    ...
    -- Los ajustes actuales se marcan como Aprobados para no afectar el historico
...
BEGIN
    ...
    BEGIN
        INSERT INTO MODELO_PRESUPUESTO_AJUSTES
            (NombrePresupuesto, CodAlmacen, Mes, Canal, Tipo, MetodoAjuste, ValorAjuste, MetodoDistribucion, Usuario, Motivo, Estado)
        VALUES
            (@NombrePresupuesto, @CodAlmacen, @Mes, @Canal, @Tipo, @MetodoAjuste, @ValorAjuste, @MetodoDistribucion, @Usuario, @Motivo, @Estado);
