-- Añadir UsuarioAprueba y Estado si no existen
IF NOT EXISTS (
    SELECT 1 
...
BEGIN
    ALTER TABLE MODELO_PRESUPUESTO_AJUSTES ADD Estado VARCHAR(20) DEFAULT ('Pendiente');
    EXEC('UPDATE MODELO_PRESUPUESTO_AJUSTES SET Estado = ''Aprobado'' WHERE Estado IS NULL OR Estado = ''Pendiente''');
END

IF NOT EXISTS (
    SELECT 1 
    FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_NAME = 'MODELO_PRESUPUESTO_AJUSTES' AND COLUMN_NAME = 'UsuarioAprueba'
)
BEGIN
    ALTER TABLE MODELO_PRESUPUESTO_AJUSTES ADD UsuarioAprueba NVARCHAR(200) NULL;
END
GO

-- Actualizar SP_AJUSTAR_PRESUPUESTO
ALTER PROCEDURE [dbo].[SP_AJUSTAR_PRESUPUESTO]
    ...
    @Estado               VARCHAR(20) = 'Aprobado', -- 'Pendiente' / 'Aprobado'
    @UsuarioAprueba       NVARCHAR(200) = NULL
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;

    -- Si solo es pendiente y no es preview, solo insertamos
    IF @Estado = 'Pendiente' AND @SoloPreview = 0
    BEGIN
        INSERT INTO MODELO_PRESUPUESTO_AJUSTES
            (NombrePresupuesto, CodAlmacen, Mes, Canal, Tipo, MetodoAjuste, ValorAjuste, MetodoDistribucion, Usuario, Motivo, Estado, UsuarioAprueba)
        VALUES
            (@NombrePresupuesto, @CodAlmacen, @Mes, @Canal, @Tipo, @MetodoAjuste, @ValorAjuste, @MetodoDistribucion, @Usuario, @Motivo, @Estado, @UsuarioAprueba);
            
        SELECT 'OK' AS Status,
               NULL AS TotalAnterior,
               NULL AS TotalNuevo;
        RETURN;
    END
    ...
    VALUES
        (@NombrePresupuesto, @Usuario, 'Ajuste', @CodAlmacen, @Mes, @Canal, @Tipo,
         ...
         '{"metodo":"' + @MetodoAjuste + '","valor":' + CAST(@ValorAjuste AS NVARCHAR(50)) + ',"distribucion":"' + @MetodoDistribucion + '","estado":"' + @Estado + '"}');
