...
BEGIN
    ...
    INSERT INTO #Canales VALUES (N'AutoPollo'),(N'ECommerce'),(N'Express'),(N'Llevar'),(N'Sal├│n'),(N'UberEats');
    ...
    CROSS APPLY (VALUES
        (N'Sal├│n', cm.SALON),(N'Llevar', cm.LLEVAR),(N'AutoPollo', cm.AUTO),
    ...
    -- Map by occurrence: 1st Monday of Feb 2026 ÔåÆ 1st Monday of Feb 2025
    ...
    ;WITH ETarget AS (
        SELECT fT.IDEVENTO,
            ...
            CanalRule = CASE
                ...
                WHEN UPPER(LTRIM(RTRIM(CONVERT(NVARCHAR(200),fT.Canal)))) IN (N'SALON',N'SAL├ôN') THEN N'Sal├│n'
    ...
    SELECT CAST(v.FECHA AS DATE),
        ...
        CASE WHEN UPPER(LTRIM(RTRIM(v.CANAL))) IN (N'AUTO',N'AUTOPOLLO',N'AUTO POLLO') THEN N'AutoPollo'
             ...
             WHEN UPPER(LTRIM(RTRIM(v.CANAL))) IN (N'SALON',N'SAL├ôN') THEN N'Sal├│n'
    ...
    -- When coverage is SPARSE (<=25%), the pattern is unreliable ÔÇö use uniform distribution.
    ...
    ;WITH Rules AS (
        SELECT IDEVENTO=f.IDEVENTO, FechaX=CAST(f.FECHA AS DATE), FechaE=CAST(f.FECHA_EFECTIVA AS DATE),
            CanalRule = CASE
                ...
                WHEN UPPER(LTRIM(RTRIM(CONVERT(NVARCHAR(200),f.Canal)))) IN (N'SALON',N'SAL├ôN') THEN N'Sal├│n'
    ...
    BEGIN
        ...
        -- Add delta to source day (FechaX) ÔÇö only for events 24,25 which exclude FechaX from targets
        INSERT INTO #DistRows(CodAlmacen,Canal,Fecha,DeltaMonto)
    ...
    INSERT INTO [' + @TablaDestino + '] (
        ...
        CodAlmacen, Participacion, Canal, A├▒o, Tipo,
    ...
    BEGIN
        ...
        SELECT
            ...
            SUM(r.Monto), g.CodGrupo, 0, r.Canal, MIN(r.A├▒o), r.Tipo,
        ...
        SELECT
            ...
            SUM(Monto), CodAlmacen, 0, N''Todos'', MIN(A├▒o), Tipo,
        ...
        SELECT v.Fecha,v.idLocal,v.[Local],v.Serie,v.idDia,v.Dia,v.Mes,
            ...
            v.CodAlmacen,0,v.Canal,v.A├▒o,N''TQP'',
        ...
        SELECT
            ...
            N''G30'', Participacion, Canal, A├▒o, Tipo,
            FechaAnterior, MontoAnterior, ParticipacionAnterior,
            FechaAnteriorAjustada, MontoAnteriorAjustado, ParticipacionAnteriorAjustado,
            MontoReal, ParticipacionReal, NombrePresupuesto
        FROM [' + @TablaDestino + ']
    ...
    SET @SQL = N'
    UPDATE t SET
        Llave_Presupuesto = t.CodAlmacen + N''_'' + CAST(t.A├▒o AS NVARCHAR(4)) + N''_'' + CAST(t.Mes AS NVARCHAR(2)) + N''_'' + CAST(DAY(t.Fecha) AS NVARCHAR(2)) + N''_'' + t.Canal + N''_'' + t.Tipo + N''_Presupuesto'',
        Llave_A├▒oAnterior = CASE WHEN t.FechaAnterior IS NOT NULL THEN t.CodAlmacen + N''_'' + CAST(YEAR(t.FechaAnterior) AS NVARCHAR(4)) + N''_'' + CAST(MONTH(t.FechaAnterior) AS NVARCHAR(2)) + N''_'' + CAST(DAY(t.FechaAnterior) AS NVARCHAR(2)) + N''_'' + t.Canal + N''_'' + t.Tipo + N''_A├▒oAnterior'' ELSE NULL END,
        Llave_AnoAnterior_Ajustado = CASE WHEN t.FechaAnteriorAjustada IS NOT NULL THEN t.CodAlmacen + N''_'' + CAST(YEAR(t.FechaAnteriorAjustada) AS NVARCHAR(4)) + N''_'' + CAST(MONTH(t.FechaAnteriorAjustada) AS NVARCHAR(2)) + N''_'' + CAST(DAY(t.FechaAnteriorAjustada) AS NVARCHAR(2)) + N''_'' + t.Canal + N''_'' + t.Tipo + N''_A├▒oAnteriorAjustado'' ELSE NULL END
    FROM [' + @TablaDestino + '] t
    WHERE t.NombrePresupuesto=@nombre;';
    EXEC sp_executesql @SQL, N'@nombre NVARCHAR(100)', @nombre=@NombrePresupuesto;
    ...
    BEGIN
        ...
        SELECT
            ...
            v.CodAlmacen, 0, N''Todos'', v.A├▒o, N''TQP'',
    ...
    VALUES(@NombrePresupuesto,@Usuario,'Recalculo',
        ...
        @CodAlmacen,@Mes,
        CASE WHEN @Usuario='JOB' THEN 'Ejecuci├│n autom├ítica diaria' ELSE 'Rec├ílculo manual' END,
