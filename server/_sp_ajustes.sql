...
BEGIN
    ...
    BEGIN
        INSERT INTO MODELO_PRESUPUESTO_AJUSTES
            (NombrePresupuesto, CodAlmacen, Mes, Canal, Tipo, MetodoAjuste, ValorAjuste, MetodoDistribucion, Usuario, Motivo, Estado, UsuarioAprueba, FechaAprobacion)
        VALUES
            (@NombrePresupuesto, @CodAlmacen, @Mes, @Canal, @Tipo, @MetodoAjuste, @ValorAjuste, @MetodoDistribucion, @Usuario, @Motivo, @Estado, @UsuarioAprueba, CASE WHEN @Estado = 'Aprobado' THEN GETDATE() ELSE NULL END);
