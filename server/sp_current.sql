ALTER PROCEDURE [dbo].[SP_CALCULAR_PRESUPUESTO]
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM dbo.KpisRosti_GruposAlmacenCab cab WITH (NOLOCK)
        JOIN dbo.KpisRosti_GruposAlmacenLin lin WITH (NOLOCK) ON lin.IDGRUPO = cab.IDGRUPO
    ...
    ;WITH ETarget AS (
        SELECT fT.IDEVENTO,
            ...
            END,
            StoreAlmacen = fT.CodAlmacen,
            GrupoAlmacen = fT.GrupoAlmacen,
            Prio = (CASE WHEN fT.Canal IS NULL OR LTRIM(RTRIM(CONVERT(NVARCHAR(200),fT.Canal)))=N'' THEN 0 ELSE 4 END)
                 + (CASE WHEN fT.CodAlmacen IS NULL THEN 0 ELSE 2 END)
    ...
    ExpandCod AS (
        SELECT e.IDEVENTO,e.FechaTarget,e.FechaBase,e.CanalRule,e.StoreAlmacen,e.GrupoAlmacen,e.Prio,
            CodAlmacen = CASE WHEN e.StoreAlmacen IS NOT NULL THEN e.StoreAlmacen
                              WHEN e.GrupoAlmacen IS NOT NULL THEN gm.CodAlmacen 
                              ELSE c.CodAlmacen END
        ...
        WHERE e.FechaBase IS NOT NULL 
          AND (
               (e.StoreAlmacen IS NOT NULL AND e.StoreAlmacen = c.CodAlmacen) OR
               (e.StoreAlmacen IS NULL AND e.GrupoAlmacen IS NOT NULL AND gm.CodAlmacen = c.CodAlmacen) OR
               (e.StoreAlmacen IS NULL AND e.GrupoAlmacen IS NULL)
          )
    ),
    ExpandCanal AS (
    ...
    ;WITH Rules AS (
        SELECT IDEVENTO=f.IDEVENTO, FechaX=CAST(f.FECHA AS DATE), FechaE=CAST(f.FECHA_EFECTIVA AS DATE),
            ...
            END,
            StoreAlmacen=f.CodAlmacen,
            GrupoAlmacen=f.GrupoAlmacen,
            Prio = (CASE WHEN f.Canal IS NULL OR LTRIM(RTRIM(CONVERT(NVARCHAR(200),f.Canal)))=N'' THEN 0 ELSE 4 END)
                 + (CASE WHEN f.CodAlmacen IS NULL THEN 0 ELSE 2 END)
    ...
    ExpandCod AS (
        SELECT r.IDEVENTO,r.FechaX,r.FechaE,r.CanalRule,r.StoreAlmacen,r.GrupoAlmacen,r.Prio,
            CodAlmacen = CASE WHEN r.StoreAlmacen IS NOT NULL THEN r.StoreAlmacen
                              WHEN r.GrupoAlmacen IS NOT NULL THEN gm.CodAlmacen 
                              ELSE c.CodAlmacen END
        FROM Rules r CROSS JOIN (SELECT CodAlmacen FROM #Cods) c
        LEFT JOIN #GrupoMiembros gm ON gm.IDGRUPO=r.GrupoAlmacen
        WHERE (
               (r.StoreAlmacen IS NOT NULL AND r.StoreAlmacen = c.CodAlmacen) OR
               (r.StoreAlmacen IS NULL AND r.GrupoAlmacen IS NOT NULL AND gm.CodAlmacen = c.CodAlmacen) OR
               (r.StoreAlmacen IS NULL AND r.GrupoAlmacen IS NULL)
        )
    ),
    ExpandCanal AS (
        SELECT CodAlmacen, Canal=CASE WHEN CanalRule IS NULL THEN k.Canal ELSE CanalRule END,
